...
@section('content')
    <main class="dashboard-content">
        <div class="container-fluid px-3 px-lg-4 py-4">
            <h1 class="h3 mb-1">{{ __("messages.edit_city") }}</h1>
            ...
            <form class="panel mt-3" method="POST" action="{{ route('cities.update', ['locale' => app()->getLocale(), 'city' => $city->id]) }}">
                @csrf @method('PUT')
                <div class="panel-body row g-3">
                    ...
                    <div class="col-12 text-end">
                        <button type="submit" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i> {{ __("messages.update_city") }}</button>
                    </div>
                </div>
            </form>
        </div>
